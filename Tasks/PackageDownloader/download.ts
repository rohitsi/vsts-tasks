...
export async function downloadPackage(coreApi: corem.ICoreApi, feedId: string, packageId: string, version: string, downloadPath: string) {
	var packageUrl = await getPackageUrl(coreApi.vsoClient, feedId, packageId);
	...
	await new Promise((resolve, reject) => {
		coreApi.restClient.get(packageUrl, ApiVersion, null, { responseIsCollection: false }, async function (error, status, result) {
			...
			if (packageType == "nuget") {
				...
				var zipLocation = path.join(downloadPath, packageName) + ".zip"
				var unzipLocation = path.join(downloadPath, packageName);
...
export async function getPackageUrl(vsoClient: vsom.VsoClient, feedId: string, packageId: string): Promise<string> {
